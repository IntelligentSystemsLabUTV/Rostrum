/**
 * ESSLD GPU ROS 2 node implementation.
 *
...
ESSLD::ESSLD(const rclcpp::NodeOptions & node_options)
: NodeBase("essld", node_options, true)
{
  dua_init_node();
  ...
  detector_config.mask_threshold = mask_threshold_;
  detector_config.median_filter_sizes = fusion_median_filter_sizes_;
  detector_config.canny_fusion_weights = fusion_canny_fusion_weights_;
  detector_config.confidence_map_sigmas = fusion_confidence_map_sigmas_;
  detector_config.confidence_map_weights = fusion_confidence_map_weights_;
  detector_config.fused_map_final_threshold = static_cast<int>(fusion_fused_map_final_threshold_);
  detector_config.hough_threshold = static_cast<int>(fusion_hough_threshold_);
  detector_config.hough_min_line_length = static_cast<int>(fusion_hough_min_line_length_);
  detector_config.hough_max_line_gap = static_cast<int>(fusion_hough_max_line_gap_);
  detector_config.grad_score = fusion_grad_score_;
  detector_config.smoothing_alpha = smoothing_alpha_;
  detector_config.hold_last_on_failure = smoothing_hold_last_on_failure_;
  ...
  detector_ = std::make_unique<ESSLDDetector>(engine_, detector_config);

  running_ = autostart_;
  write_csv_headers();
  image_processing_thread_ = std::thread(&ESSLD::image_processing_loop, this);
  RCLCPP_INFO(this->get_logger(), "ESSLD GPU node initialized");
}
...
void ESSLD::init_publishers()
{
  ...
  if (publish_mask_) {
    mask_pub_ = std::make_shared<image_transport::Publisher>(
      image_transport::create_publisher(
        this,
        "~/mask",
        dua_qos::Reliable::get_image_qos().get_rmw_qos_profile()));
    RCLCPP_INFO(this->get_logger(), "[TOPIC PUB] '%s' (image_transport)",
      mask_pub_->getTopic().c_str());
  }
}

void ESSLD::init_subscribers()
{
  ...
  image_sub_ = std::make_shared<image_transport::Subscriber>(
    image_transport::create_subscription(
      ...
      std::bind(&ESSLD::callback_image, this, std::placeholders::_1),
...
void ESSLD::callback_image(const Image::ConstSharedPtr & image_msg)
{
  if (!running_) {
    return;
  }
  ...
  if (dropped) {
    RCLCPP_WARN_THROTTLE(
      ...
      *this->get_clock(),
      5000,
      "ESSLD image processing queue full; dropped %zu old image(s). "
      "Increase subscribers.depth or lower the input FPS if every frame must be processed.",
      dropped_images);
  }
}

void ESSLD::image_processing_loop()
{
...
void ESSLD::process_image(const Image::ConstSharedPtr & image_msg)
{
  cv::Mat frame;
  try {
  ...
  try {
    ...
    RCLCPP_ERROR(this->get_logger(), "ESSLD detection failed: %s", ex.what());
    return;
  }

  Float64MultiArray msg;
  msg.data = {result.y, result.theta_deg, result.elapsed_s * 1000.0, result.valid ? 1.0 : 0.0};
  ...
  if (publish_mask_ && mask_pub_ && !result.mask.empty()) {
    auto mask_msg = dua_cv_bridge::frame_to_msg(result.mask, "mono8");
...
void ESSLD::append_state_csv(const FrameResult & result)
{
  if (!states_csv_.is_open()) {
    return;
  }
...
void ESSLD::append_frame_timing_csv(const FrameResult & result)
{
  if (!frame_timing_csv_.is_open()) {
    return;
  }
...
RCLCPP_COMPONENTS_REGISTER_NODE(essld_cpp::ESSLD)
